...
func _build_start_layer() -> void:
	...
	var start_style = host.ui_factory.panel_style(Color("070a0875"), 0, 0, Color.TRANSPARENT, 38, 34)
	...
	var name_prompt = host.ui_factory.text(content, host._ui_text("player_name_prompt"), true, 13)
	...
	var name_style = host.ui_factory.input_style(Color("111812c2"), Color(host.COLORS.line, 0.58))
	name_style.set_corner_radius_all(1)
	host.name_input.add_theme_stylebox_override("normal", name_style)
	var name_focus_style = host.ui_factory.input_style(Color("151d17dc"), host.COLORS.accent)
	...
	host.start_begin_button = host.ui_factory.ornate_button("开始新故事", host._new_game)
	host.start_begin_button.custom_minimum_size.y = 66
	content.add_child(host.start_begin_button)
	var continue_button = host.ui_factory.utility_button("翻开旧卷", host._load_game)
	continue_button.alignment = HORIZONTAL_ALIGNMENT_LEFT
	content.add_child(continue_button)
	var start_settings_button = host.ui_factory.utility_button("大模型与体验设置", host.start_settings_screen_controller._open_audio_settings)
	start_settings_button.alignment = HORIZONTAL_ALIGNMENT_LEFT
	content.add_child(start_settings_button)
	host.retry_button = host.ui_factory.action_button("重新连接本地服务", host._retry_connection)
...
func _build_settings_layer() -> void:
	...
	card.add_theme_stylebox_override("panel", host.ui_factory.panel_style(host.COLORS.panel, 1, 14, host.COLORS.accent_pressed, 28, 24))
	center.add_child(card)
	...
	host.ui_factory.text(host.settings_box, "体验设置", false, 25)
	host.ui_factory.text(host.settings_box, "管理显示、声音、诊断、人物对话与世界导演。", true, 14)
	...
	var display_heading = host.ui_factory.text(display_box, "显示", false, host.TYPE_SCALE.section)
	...
	host.display_status_label = host.ui_factory.text(display_box, "", true, host.TYPE_SCALE.meta)
	...
	host.motion_button = host.ui_factory.action_button("动态效果 · 开启", host.start_settings_screen_controller._toggle_motion)
	...
	var audio_heading = host.ui_factory.text(audio_box, "声音", false, host.TYPE_SCALE.section)
	...
	audio_box.add_child(host.ui_factory.action_button("全部静音", host.start_settings_screen_controller._toggle_sound))
	...
	host.log_level_button = host.ui_factory.action_button("日志等级 · %s" % host.log_level, host.display_settings_controller._cycle_log_level)
	...
	var log_folder_button = host.ui_factory.action_button("打开日志", host.runtime_logger_controller._open_log_folder)
	log_folder_button.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	diagnostic_row.add_child(log_folder_button)
	var diagnostics_button = host.ui_factory.action_button("导出诊断包", host.runtime_logger_controller._export_diagnostics)
	...
	var ai_intro = host.ui_factory.text(ai_tab, "大模型人物对话与世界导演使用 Anthropic Messages 兼容接口。关闭时完全不调用模型；开启后超时、空响应、结构错误或非法导演指令都会直接报错并回滚当日推进。", true, host.TYPE_SCALE.detail)
	...
	host.ai_status_label = host.ui_factory.text(ai_tab, "", true, host.TYPE_SCALE.detail)
	...
	host.settings_box.add_child(host.ui_factory.button("返回游戏", host.start_settings_screen_controller._close_audio_settings, false))
...
func _ai_settings_input(parent: VBoxContainer, label_text: String, value: String, tooltip: String) -> LineEdit:
	...
	input.add_theme_stylebox_override("normal", host.ui_factory.input_style(host.COLORS.panel_alt, host.COLORS.line))
	input.add_theme_stylebox_override("focus", host.ui_factory.input_style(host.COLORS.bg_lift, host.COLORS.accent))
	row.add_child(input)
	return input
...
func _toggle_sound() -> void:
	...
	screen.sound_button.text = "声音" if host.sound_enabled else "声音 · 静音"
...
func _toggle_motion() -> void:
	...
	if screen.world_map_view:
		screen.world_map_view.set_motion_enabled(host.motion_enabled)
