...
“体验设置”提供窗口、无边框全屏与独占全屏模式；窗口模式可选当前显示器能容纳的输出尺寸，全屏模式使用显示器原生分辨率。界面缩放会根据当前输出尺寸过滤不安全的比例，并与显示设置一同保存在运行目录的 `settings.cfg`。

“体验设置 → 大模型”提供人物对话开关、模型名称、Anthropic Messages 兼容接口地址和遮罩 API Key 输入。配置保存到当前用户运行目录的 `ai-settings.json`，应用后立即更新本地服务，不会重置游戏进度；关闭或清除密钥后不再调用模型。
