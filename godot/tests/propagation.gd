...
func _run() -> void:
	...
	var term_text := _descendant_text(app.game_screen_controller.actor_focus_message_list) + _descendant_text(app.game_screen_controller.actor_focus_detail_box)
	...
	if not await _execute("tell:N03:F01:trust"):
		return
	if app.game_screen_controller.timing_label.text != "第21天子时 · 已核实":
		return _fail("verified timing is not visible in the header")
	if app.game_screen_controller.world_map_view.travel_end_day < 1 or app.game_screen_controller.world_map_view.travel_active or app.presentation_busy:
		return _fail("map travel animation did not finish cleanly")
	if app.audio_director.current_scene_key != "qinglan" or not app.game_screen_controller.location_stage.has_formal_asset():
		return _fail("Qinglan presentation profile did not become active")
	if not app.game_screen_controller.actor_portrait_frame.visible or app.game_screen_controller.actor_portrait.texture == null:
	...
	if app.actor_expression_by_id.get("N03", "") != "troubled" or app.game_screen_controller.actor_portrait.texture != shen_profile.troubled:
	...
	if app.game_screen_controller.action_canvas.visible or "情报因果线" not in _descendant_text(app.scene_box) or "等待公开回响" not in _descendant_text(app.scene_box):
	...
	app.action_panel_controller._focus_actor_actions("N03", "沈砚秋")
	var dossier_text: String = _descendant_text(app.game_screen_controller.actor_focus_message_list) + _descendant_text(app.game_screen_controller.actor_focus_detail_box) + str(app.game_screen_controller.objective_label.text)
	if "传播风险" not in dossier_text or "正在权衡" not in dossier_text or not app.game_screen_controller.actor_focus_workspace.visible:
	...
	if not await _execute("wait:next"):
		return
	if app.actor_expression_by_id.get("N03", "") != "decisive" or app.game_screen_controller.actor_portrait.texture != shen_profile.decisive:
	...
	if not app.causal_layer.visible or app.game_screen_controller.action_canvas.visible or app.causal_background.texture == null or app.causal_portrait.texture != shen_profile.decisive:
	...
	if app.causal_layer.visible or not app.game_screen_controller.action_canvas.visible:
	...
	var recovery_focus_text: String = _descendant_text(app.game_screen_controller.actor_focus_message_list) + _descendant_text(app.game_screen_controller.actor_focus_detail_box) + _descendant_text(app.game_screen_controller.actor_focus_footer)
	...
	var route_text := _descendant_text(app.game_screen_controller.actor_focus_message_list) + _descendant_text(app.game_screen_controller.actor_focus_detail_box) + _descendant_text(app.game_screen_controller.actor_focus_footer)
	...
	if not app.ending_layer.visible or app.game_screen_controller.action_canvas.visible:
	...
	if "你的去向" not in _visible_descendant_text(app.ending_box):
		return _fail("ending does not show its concise player coda by default")
	...
	if expanded_ending.count("信用 +2") != 1 or "本局记录" in expanded_ending or "人物计划改写" in expanded_ending:
