...
项目已经具备数据驱动基础：场景、人物、事实、地点、物品和基础行动分别存放在 `data/blackwind/*.json`，由 `internal/scenario` 加载、校验并转换为 `domain.Bundle`。模拟内核负责确定性的时间推进、NPC 决策、移动、市场、交易和竞争；Godot 只消费应用层提供的玩家可见视图。
