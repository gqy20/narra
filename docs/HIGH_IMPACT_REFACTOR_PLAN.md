...
- 内容 Schema 已升级到 v2，并提供可预览、验证后写入、可幂等重复执行的显式逐版本迁移命令；
- API v1 响应契约可生成并验证，Godot 通过独立适配器校验版本、错误与 PlayerView 默认值；
...
内容包使用显式逐版本迁移链。默认命令只生成预览，不修改作者源；加上 `-write` 后先在临时目录验证完整内容包，再替换发生变化的文件：

```powershell
go run ./cmd/content-migrate -data data/blackwind
go run ./cmd/content-migrate -data data/blackwind -write
```

迁移报告包含起止版本、变更说明及迁移前后的文件哈希；当前版本重复执行无变化，高于引擎支持范围的版本会被拒绝。
