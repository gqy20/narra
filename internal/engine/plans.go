...
func (e *Engine) attachPlan(actorID string, strategy *domain.Strategy) {
	if !strategy.Generated || (strategy.ActionID != "buy" && strategy.ActionID != "explore") {
		return
	}
	npc := e.state.NPCs[actorID]
	if npc == nil || !wantsContest(npc) {
		return
	}
	...
	if plan == nil || plan.Status != "active" {
		plan = &domain.PlanChain{
			...
			Steps: []domain.PlanStep{{ID: "supply", Description: "取得入谷补给", Status: "pending"}, {ID: "navigate", Description: "沿路线接近目标", Status: "pending"}, {ID: "enter", Description: "进入目标地点", Status: "pending"}},
		}
		npc.Plans[plan.ID] = plan
		npc.ActivePlanID = plan.ID
	}
	stepID := ""
	if strategy.ActionID == "buy" {
		stepID = "supply"
	} else {
		stepID = "navigate"
